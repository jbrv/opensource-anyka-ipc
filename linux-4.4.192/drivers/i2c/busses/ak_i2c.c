...
static ssize_t show_i2c_clk_rate(struct device *dev, struct device_attribute *attr,char *buf){
        struct ak_i2c_dev *i2c_dev = dev_get_drvdata(dev);
		return sprintf(buf, "%u\n",(i2c_dev->bus_clk_rate*1000));
}

static ssize_t store_i2c_clk_rate(struct device *dev, struct device_attribute *attr,
	   ...
	   if(ret)
	   		...
	   		i2c_dev->bus_clk_rate = val;
			printk("set bus_clk_rate to %d.\n", i2c_dev->bus_clk_rate);
	   		ak_i2c_initialise(i2c_dev);
	   }
...
static DEVICE_ATTR(i2c_clk_rate, S_IWUSR|S_IRUSR, show_i2c_clk_rate, store_i2c_clk_rate);
...
static int ak_i2c_probe(struct platform_device *pdev)
{
	...
	if(i2c_dev->id == 0){
		int ret = sysfs_create_file(&(i2c_dev->dev->kobj),&dev_attr_i2c_clk_rate);
		if(ret == 0){
			printk("create i2c_clk_rate sysfs device attributes success\n");
		}else{
			printk("create i2c_clk_rate sysfs device attributes failed\n");
			return -1;
		}
	}
